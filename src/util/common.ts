...
export const getServerSideRequest = async <T = AnyObject>(
  args: ServerSideApiRequest<T>
) => {
...
export const runTimeSharedConfig = () => {
  ...
  if (typeof window !== 'undefined') {
    const configValues = Object.values(config.publicRuntimeConfig.env) as {
      APP_URL: string;
    }[];
    const env = configValues.find(
      (conf) =>
        conf.APP_URL.toLowerCase().split('://')[1] ===
...
export const getAppUrl = (lang: Locales, pathName: string) =>
  `${runTimeSharedConfig().APP_URL}/${lang}${pathName}`;
export const getApiUrl = (pathName: string) =>
  `${runTimeSharedConfig().BASE_API}/${pathName}`;
  ...
  return days
    .reduce(
      (ranges: any, day: any, i: any, arr: any) =>
        i && day !== arr[i - 1] + 1
          ? [...ranges, [day]]
          : (ranges[ranges.length - 1].push(day), ranges),
      [[]]
    )
    .map((range: any) => {
      if (range.length > 2) {
        return `${daysOfWeek[range[0]]} - to - ${
          daysOfWeek[range[range.length - 1]]
        }`;
...
export const getParams = (params: Record<string, any>) => {
  ...
  filteredParams.forEach(
    ([key, val]) =>
      (paramsInSnakeCase[camelToSnakeCase(key)] = encodeURIComponent(val))
  );
...
export const isTimeInRange = (startTime: string, endTime: string) => {
  ...
  const currentMilliseconds =
    currentHours * 60 * 60 * 1000 + currentMinutes * 60 * 1000;
...
export const toFormData = (
  data: Record<string, string | number | boolean | string[] | number[] | File>
) => {
